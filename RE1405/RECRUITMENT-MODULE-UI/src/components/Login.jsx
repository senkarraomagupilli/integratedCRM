...
import { addAuthData } from "../Servicescalls";
...
const Login = () => {
  ...
  const loginForm = async e => {
    ...
    try {
      const response = await addAuthData("login", bodydata)
      var _data = response
      localStorage.setItem("authUser", _data.data.token)
      navigate('/home');
  ...
  return (
    <div className="flex">
      <div className="w-3/5 bg-white h-screen">
        <Slider {...settings}>
          <div className='p-3'>
            <div className='backgroundimg' style={{ backgroundImage: `url(${firstslide})` }}>
        ...
        <div className="items-center flex flex-col justify-inherit h-screen">
          {toasterMessage && (
            <div className="bg-red-500 text-white p-2 rounded-md mb-4 relative">
              {toasterMessage}
              <button className="top-0 right-0 px-2 py-1" onClick={closeToaster}>
                X
              </button>
            </div>
          )}
          <div className={toasterMessage ? "w-96 p-7" : "w-96 p-7 mt-[100px]"}>
            ...
            <form onSubmit={(e) => { loginForm(e) }}>
              ...
              <button type="submit" className="w-80 bg-[#29AB87] p-2 mt-4 text-[#FFFFFF] rounded-md text-[18px]" >Login</button>
            </form>
          </div>
        </div>
      </div>
